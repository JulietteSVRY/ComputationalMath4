...
def gdots(func: str, a: float, b: float, count: float) -> tuple[list, list]:
    x = Symbol("x")
    func = simplify(func)
    step = (b - a) / count
...
def draw_function(
    ...
    x = Symbol("x")
    func = simplify(func)
    x_dots, y_dots = gdots(func, a, b, count)
